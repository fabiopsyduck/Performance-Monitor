...
if ($Content) {
    try {
        ...
        # Validação Freq
        if (-not $Global:ConfigData.FreqValue) { $Global:ConfigData.FreqValue = 300 }
...
# --- VARIÁVEIS DOS 4 FILTROS ATUALIZADOS ---
$SyncHash.AdaptiveTrimmedLower = $Global:ConfigData.AdaptiveTrimmedLower
$SyncHash.AdaptiveMax = $Global:ConfigData.AdaptiveMax
$SyncHash.AdaptiveTrimmedUpper = $Global:ConfigData.AdaptiveTrimmedUpper
$SyncHash.AdaptiveSimpleAvg = $Global:ConfigData.AdaptiveSimpleAvg
$SyncHash.AdaptiveEnabled = $Global:ConfigData.AdaptiveEnabled

# --- NOVA VARIÁVEL DE ESTADO DA GPU ---
$SyncHash.GpuActive = $true

# Configuração Inicial de Frequência (Prioridade: Adaptive > Freq Manual > Default)
$SyncHash.SleepTime = if ($Global:ConfigData.AdaptiveEnabled) { 
    250 # Margem de 50ms para processamento pesado dos filtros matemáticos
...
"@
    ...
    while ($Hash.Rodar) {
        ...
        try {
            ...
            # Validação rigorosa: Só aceita se NÃO for nulo, NÃO for -1, e CONTIVER o separador ';'
            if ([string]::IsNullOrWhiteSpace($gpuData) -or $gpuData -eq "-1" -or $gpuData.IndexOf(';') -eq -1) {
                $Hash.GpuActive = $false
                $Hash.GpuPercText = "--"
                $Hash.VramPercText = "--"
            } else {
                $Hash.GpuActive = $true
            ...
            $Hash.GpuActive = $false
            $Hash.GpuPercText = "Err"
            $Hash.VramPercText = "Err" 
        }
        
        # SLEEP DINAMICO INTELIGENTE
        # Só acelera se o Adaptive estiver LIGADO globalmente E um dos 4 filtros estiver ativo.
        if ($Hash.AdaptiveEnabled -and ($Hash.AdaptiveTrimmedLower -or $Hash.AdaptiveMax -or $Hash.AdaptiveTrimmedUpper -or $Hash.AdaptiveSimpleAvg)) {
            $currentSleep = [math]::Max(75, [math]::Round($Hash.SleepTime / 4))
        } else {
            $currentSleep = $Hash.SleepTime
        }
        Start-Sleep -Milliseconds $currentSleep
    }
...
function New-MonitorBlock {
    ...
    # --- ALTERAÇÃO AQUI: 'Title' e 'Symbol' adicionados ao dicionário ---
    return @{ "Bar"=$pnlFill; "LblVal"=$lblVal; "LblBottom"=$lblBottom; "MaxWidth"=280; "Frame"=$pnlFrame; "Title"=$lblTitle; "Symbol"=$lblSym }
}
...
function Apply-AdaptiveFilter {
    param($RawVal, $QueueRef, $LastRef)
    
    # Se os filtros estiverem desligados, passa direto (Agora verifica os 4 métodos)
    if (-not $Global:ConfigData.AdaptiveTrimmedLower -and -not $Global:ConfigData.AdaptiveMax -and -not $Global:ConfigData.AdaptiveTrimmedUpper -and -not $Global:ConfigData.AdaptiveSimpleAvg) {
        $LastRef.Value = $RawVal
        return $RawVal
    }

    # 1. Enfileira a nova micro-leitura
    $QueueRef.Enqueue($RawVal)

    # 2. Se a caixa não tem 4 amostras, devolvemos a memória antiga
    if ($QueueRef.Count -lt 4) {
        if ($LastRef.Value -eq 0 -and $RawVal -ne 0) { $LastRef.Value = $RawVal; return $RawVal }
        return $LastRef.Value
    }

    # 3. Bateu 4 amostras. Hora da estatística final!
    $arr = $QueueRef.ToArray()
    $QueueRef.Clear() 

    if ($Global:ConfigData.AdaptiveTrimmedLower) {
        # Trimmed Mean (Lower Focus): Corta o maior valor (índice 3), faz média de 0, 1, 2
        $arr = $arr | Sort-Object
        $calc = ($arr[0] + $arr[1] + $arr[2]) / 3
        $Result = [math]::Round($calc)
    } 
    elseif ($Global:ConfigData.AdaptiveMax) {
        # Max Pooling: Pega estritamente o maior valor
        $Result = ($arr | Measure-Object -Maximum).Maximum
    }
    elseif ($Global:ConfigData.AdaptiveTrimmedUpper) {
        # Trimmed Mean (Upper Focus): Corta o menor valor (índice 0), faz média de 1, 2, 3
        $arr = $arr | Sort-Object
        $calc = ($arr[1] + $arr[2] + $arr[3]) / 3
        $Result = [math]::Round($calc)
    }
    elseif ($Global:ConfigData.AdaptiveSimpleAvg) {
        # Standard Average: Média simples de todos os 4 valores
        $calc = ($arr[0] + $arr[1] + $arr[2] + $arr[3]) / 4
        $Result = [math]::Round($calc)
    }

    $LastRef.Value = $Result
    return $Result
}
...
function Show-FreqSettings {
    ...
    $btnReset = New-Object System.Windows.Forms.Button; $btnReset.Text = "Reset (300ms)"; $btnReset.Location = "20, 100"; $btnReset.Size = "120, 30"; $btnReset.FlatStyle = "Flat"
    ...
    $btnReset.Add_Click({ $trackFreq.Value = 300; & $UpdateFreqUI })
...
function Show-AdvancedSettings {
    # 1. JANELA BLINDADA COMPACTADA (Altura 490 para caber os 4 filtros)
    $frmAdv = New-Object System.Windows.Forms.Form; $frmAdv.Size = New-Object System.Drawing.Size(920, 490); $frmAdv.StartPosition = "CenterScreen"; $frmAdv.TopMost = $true; 
    $frmAdv.FormBorderStyle = "None"; $frmAdv.BackColor = [System.Drawing.Color]::DimGray; $frmAdv.AutoScaleMode = [System.Windows.Forms.AutoScaleMode]::None
    ...
    # --- MAIN CONTENT PANEL ---
    $pnlAdvContent = New-Object System.Windows.Forms.Panel; $pnlAdvContent.Size = "918, 459"; $pnlAdvContent.Location = "1, 30"; $pnlAdvContent.BackColor = [System.Drawing.Color]::WhiteSmoke; $frmAdv.Controls.Add($pnlAdvContent)

    # TEXTO DINÂMICO DA FREQUÊNCIA
    $internalFreq = [math]::Max(75, [math]::Round($Global:ConfigData.FreqValue / 4))
    $lblFreqInfo = New-Object System.Windows.Forms.Label; $lblFreqInfo.Text = "UI Update Frequency: $($Global:ConfigData.FreqValue)ms (Internal Sampling: ${internalFreq}ms)"; $lblFreqInfo.Location = "10, 15"; $lblFreqInfo.AutoSize = $true; $lblFreqInfo.Font = New-Object System.Drawing.Font("Segoe UI", 14, [System.Drawing.FontStyle]::Bold, [System.Drawing.GraphicsUnit]::Pixel); $lblFreqInfo.ForeColor = [System.Drawing.Color]::DarkSlateBlue; $pnlAdvContent.Controls.Add($lblFreqInfo)
    
    $lblInfo = New-Object System.Windows.Forms.Label; $lblInfo.Text = "Filters collect 4 rapid samples internally to ensure extreme precision on the screen."; $lblInfo.Location = "10, 40"; $lblInfo.AutoSize = $true; $lblInfo.ForeColor = [System.Drawing.Color]::Gray; $pnlAdvContent.Controls.Add($lblInfo)

    # TARGET COMPONENTS E APPLY TO
    ...
    # PIPELINE GROUP
    $grpM = New-Object System.Windows.Forms.GroupBox; $grpM.Text = "Filtering Method (Discrete Window Sampling):"; $grpM.Location = "10, 230"; $grpM.Size = "880, 140" 
    ...
    $btnHelp.Add_Click({ 
        $frmHelp = New-Object System.Windows.Forms.Form; $frmHelp.Size = New-Object System.Drawing.Size(650, 520); $frmHelp.StartPosition = "CenterParent"; $frmHelp.TopMost = $true; $frmHelp.FormBorderStyle = "None"; $frmHelp.BackColor = [System.Drawing.Color]::DimGray; $frmHelp.AutoScaleMode = [System.Windows.Forms.AutoScaleMode]::None; $frmHelp.Font = New-Object System.Drawing.Font("Segoe UI", 12, [System.Drawing.FontStyle]::Regular, [System.Drawing.GraphicsUnit]::Pixel)
        $pnlHelpHeader = New-Object System.Windows.Forms.Panel; $pnlHelpHeader.Size = "650, 30"; $pnlHelpHeader.Location = "0, 0"; $pnlHelpHeader.BackColor = [System.Drawing.Color]::FromArgb(40, 40, 40); $frmHelp.Controls.Add($pnlHelpHeader)
        $lblHelpHeader = New-Object System.Windows.Forms.Label; $lblHelpHeader.Text = "Discrete Sampling Details"; $lblHelpHeader.Location = "10, 7"; $lblHelpHeader.AutoSize = $true; $lblHelpHeader.ForeColor = [System.Drawing.Color]::White; $lblHelpHeader.Font = New-Object System.Drawing.Font("Segoe UI", 13, [System.Drawing.FontStyle]::Bold, [System.Drawing.GraphicsUnit]::Pixel); $pnlHelpHeader.Controls.Add($lblHelpHeader)
        $btnHelpClose = New-Object System.Windows.Forms.Button; $btnHelpClose.Text = "X"; $btnHelpClose.Size = "40, 30"; $btnHelpClose.Location = "610, 0"; $btnHelpClose.FlatStyle = "Flat"; $btnHelpClose.FlatAppearance.BorderSize = 0; $btnHelpClose.ForeColor = [System.Drawing.Color]::White; $btnHelpClose.BackColor = [System.Drawing.Color]::Transparent; $btnHelpClose.Cursor = [System.Windows.Forms.Cursors]::Hand; $pnlHelpHeader.Controls.Add($btnHelpClose)
        ...
        $pnlHelpContent = New-Object System.Windows.Forms.Panel; $pnlHelpContent.Size = "648, 489"; $pnlHelpContent.Location = "1, 30"; $pnlHelpContent.BackColor = [System.Drawing.Color]::WhiteSmoke; $frmHelp.Controls.Add($pnlHelpContent)
        
        $lblMsg = New-Object System.Windows.Forms.Label; 
        $lblMsg.Text = "The internal engine collects 4 rapid hardware samples per UI update cycle.`n`n1. Trimmed Mean (Lower Focus)`nHow it works: Filters out sudden transient spikes by discarding the highest reading and averaging the rest. Delivers a perfectly smooth but highly accurate line.`n`n2. Trimmed Mean (Upper Focus)`nHow it works: Mitigates the impact of split-second idle moments by discarding the minimum value and averaging the rest. Delivers a smooth line focused on actual active processing.`n`n3. Max Pooling`nHow it works: Captures and outputs only the absolute highest value in the sample window. This guarantees no micro-stutter or sudden usage spike goes unnoticed.`n`n4. Standard Average`nHow it works: Processes all 4 readings equally to calculate a standard mathematical average. Best for a generalized overview without any data exclusion."
        $lblMsg.Location = "20, 20"; $lblMsg.Size = "610, 400"; $lblMsg.AutoSize = $false; $lblMsg.Font = New-Object System.Drawing.Font("Segoe UI", 12, [System.Drawing.FontStyle]::Regular, [System.Drawing.GraphicsUnit]::Pixel); $pnlHelpContent.Controls.Add($lblMsg)
        
        $btnHelpOk = New-Object System.Windows.Forms.Button; $btnHelpOk.Text = "OK"; $btnHelpOk.Location = "265, 435"; $btnHelpOk.Size = "120, 30"; $btnHelpOk.BackColor = [System.Drawing.Color]::DodgerBlue; $btnHelpOk.ForeColor = [System.Drawing.Color]::White; $btnHelpOk.FlatStyle = "Flat"; $btnHelpOk.Add_Click({ $frmHelp.Close() }); $pnlHelpContent.Controls.Add($btnHelpOk)
        [void]$frmHelp.ShowDialog()
    })
    $grpM.Controls.Add($btnHelp)

    # OS 4 FILTROS DE SELEÇÃO (AGORA REORDENADOS COM FOCO MATEMÁTICO)
    $chkM1 = New-Object System.Windows.Forms.CheckBox; $chkM1.Text = "Trimmed Mean (Lower Focus)"; $chkM1.Location = "20, 25"; $chkM1.AutoSize = $true; $chkM1.Checked = $Global:ConfigData.AdaptiveTrimmedLower; $chkM1.FlatStyle = "System"; $grpM.Controls.Add($chkM1)
    $lblM1 = New-Object System.Windows.Forms.Label; $lblM1.Text = "(Smooth line: Discards the highest value, averages the remaining 3)"; $lblM1.Location = "220, 26"; $lblM1.AutoSize = $true; $lblM1.Font = New-Object System.Drawing.Font("Segoe UI", 11, [System.Drawing.FontStyle]::Regular, [System.Drawing.GraphicsUnit]::Pixel); $lblM1.ForeColor = [System.Drawing.Color]::DimGray; $grpM.Controls.Add($lblM1)
    
    $chkM3 = New-Object System.Windows.Forms.CheckBox; $chkM3.Text = "Trimmed Mean (Upper Focus)"; $chkM3.Location = "20, 50"; $chkM3.AutoSize = $true; $chkM3.Checked = $Global:ConfigData.AdaptiveTrimmedUpper; $chkM3.FlatStyle = "System"; $grpM.Controls.Add($chkM3)
    $lblM3 = New-Object System.Windows.Forms.Label; $lblM3.Text = "(Sustained load: Discards the lowest value, averages the remaining 3)"; $lblM3.Location = "220, 51"; $lblM3.AutoSize = $true; $lblM3.Font = New-Object System.Drawing.Font("Segoe UI", 11, [System.Drawing.FontStyle]::Regular, [System.Drawing.GraphicsUnit]::Pixel); $lblM3.ForeColor = [System.Drawing.Color]::DimGray; $grpM.Controls.Add($lblM3)

    $chkM2 = New-Object System.Windows.Forms.CheckBox; $chkM2.Text = "Max Pooling"; $chkM2.Location = "20, 75"; $chkM2.AutoSize = $true; $chkM2.Checked = $Global:ConfigData.AdaptiveMax; $chkM2.FlatStyle = "System"; $grpM.Controls.Add($chkM2)
    $lblM2 = New-Object System.Windows.Forms.Label; $lblM2.Text = "(Absolute precision: Strictly displays the highest peak)"; $lblM2.Location = "220, 76"; $lblM2.AutoSize = $true; $lblM2.Font = New-Object System.Drawing.Font("Segoe UI", 11, [System.Drawing.FontStyle]::Regular, [System.Drawing.GraphicsUnit]::Pixel); $lblM2.ForeColor = [System.Drawing.Color]::DimGray; $grpM.Controls.Add($lblM2)

    $chkM4 = New-Object System.Windows.Forms.CheckBox; $chkM4.Text = "Standard Average"; $chkM4.Location = "20, 100"; $chkM4.AutoSize = $true; $chkM4.Checked = $Global:ConfigData.AdaptiveSimpleAvg; $chkM4.FlatStyle = "System"; $grpM.Controls.Add($chkM4)
    $lblM4 = New-Object System.Windows.Forms.Label; $lblM4.Text = "(Standard: Averages all 4 samples equally)"; $lblM4.Location = "220, 101"; $lblM4.AutoSize = $true; $lblM4.Font = New-Object System.Drawing.Font("Segoe UI", 11, [System.Drawing.FontStyle]::Regular, [System.Drawing.GraphicsUnit]::Pixel); $lblM4.ForeColor = [System.Drawing.Color]::DimGray; $grpM.Controls.Add($lblM4)

    $pnlAdvContent.Controls.Add($grpM)

    # LÓGICA DE EXCLUSÃO MÚTUA PARA OS 4 FILTROS
    $chkM1.Add_Click({ if ($chkM1.Checked) { $chkM2.Checked=$false; $chkM3.Checked=$false; $chkM4.Checked=$false } })
    $chkM2.Add_Click({ if ($chkM2.Checked) { $chkM1.Checked=$false; $chkM3.Checked=$false; $chkM4.Checked=$false } })
    $chkM3.Add_Click({ if ($chkM3.Checked) { $chkM1.Checked=$false; $chkM2.Checked=$false; $chkM4.Checked=$false } })
    $chkM4.Add_Click({ if ($chkM4.Checked) { $chkM1.Checked=$false; $chkM2.Checked=$false; $chkM3.Checked=$false } })

    # BOTÕES
    $btnCancel = New-Object System.Windows.Forms.Button; $btnCancel.Text = "Cancel"; $btnCancel.Location = "330, 400"; $btnCancel.Size = "130, 30"; $btnCancel.BackColor = [System.Drawing.Color]::Firebrick; $btnCancel.ForeColor = [System.Drawing.Color]::White; $btnCancel.FlatStyle = "Flat"; $btnCancel.Add_Click({ $frmAdv.Close() }); $pnlAdvContent.Controls.Add($btnCancel)
    $btnOk = New-Object System.Windows.Forms.Button; $btnOk.Text = "Done"; $btnOk.Location = "480, 400"; $btnOk.Size = "130, 30"; $btnOk.BackColor = [System.Drawing.Color]::LightGray; $btnOk.ForeColor = [System.Drawing.Color]::Gray; $btnOk.FlatStyle = "Flat"; $btnOk.Enabled = $false
    ...
    $ValidateForm = {
        ...
        $mainFilterSelected = ($chkM1.Checked -or $chkM2.Checked -or $chkM3.Checked -or $chkM4.Checked)
        if ($validTargets -and $validApply -and $mainFilterSelected) { $btnOk.BackColor = [System.Drawing.Color]::SeaGreen; $btnOk.ForeColor = [System.Drawing.Color]::White; $btnOk.Enabled = $true } else { $btnOk.BackColor = [System.Drawing.Color]::LightGray; $btnOk.ForeColor = [System.Drawing.Color]::Gray; $btnOk.Enabled = $false }
    }
    ...
    $chkM1.Add_CheckedChanged($ValidateForm); $chkM2.Add_CheckedChanged($ValidateForm); $chkM3.Add_CheckedChanged($ValidateForm); $chkM4.Add_CheckedChanged($ValidateForm)
    & $ValidateForm
    ...
    $btnOk.Add_Click({
        ...
        $Global:ConfigData.AdaptiveTrimmedLower = $chkM1.Checked
        $Global:ConfigData.AdaptiveMax = $chkM2.Checked
        $Global:ConfigData.AdaptiveTrimmedUpper = $chkM3.Checked
        $Global:ConfigData.AdaptiveSimpleAvg = $chkM4.Checked
        
        # Manda a ordem de aceleração para a Thread Secundária
        $SyncHash.AdaptiveTrimmedLower = $chkM1.Checked
        $SyncHash.AdaptiveMax = $chkM2.Checked
        $SyncHash.AdaptiveTrimmedUpper = $chkM3.Checked
        $SyncHash.AdaptiveSimpleAvg = $chkM4.Checked
        ...
        # TEXTO DE AVISO DINÂMICO APLICADO AQUI
        $lblDoneMsg = New-Object System.Windows.Forms.Label; $lblDoneMsg.Text = "Settings applied! Click 'SAVE CONFIGURATION' to keep them.`nBase sampling frequency dynamically adjusted."; $lblDoneMsg.Location = "20, 20"; $lblDoneMsg.Size = "360, 40"; $lblDoneMsg.AutoSize = $false; $pnlDoneContent.Controls.Add($lblDoneMsg)
...
$OpenConfig = {
    ...
    $lblVersion.Text = "Performance Monitor v1.0  |  Developed by Fabiopsyduck"
    ...
    $UpdateBtnStates = {
        ...
        $isFreqChanged = ($chkFreq.Checked -ne $Global:ConfigData.FreqEnabled) -or ($Global:ConfigData.FreqValue -ne 300)
        ...
        # ---> AQUI: $isSpikeChanged FOI REMOVIDO DA CONDIÇÃO IF <---
        if ($isFactBack -or $isFactCont -or $isFactPos -or $isFactLock -or $isFactHidden -or $isFactReqAdmin -or $isFactAdapt -or $isFactHot -or $isFreqChanged -or $isMinChanged -or $isCloseChanged -or $isFactClose -or $isFactTask) { 
    ...
    $btnSave.Add_Click({ 
        ...
        $SaveData = @{ 
            ...
            AdaptiveTrimmed = $Global:ConfigData.AdaptiveTrimmed; AdaptiveMax = $Global:ConfigData.AdaptiveMax; 
        ...
        # Sincroniza o botão de liga/desliga com o motor em tempo real:
        $SyncHash.AdaptiveEnabled = $chkAdaptive.Checked 
        $Global:HasPreviousConfig = $true; 
...
$script:UpdateBlock = {
    param($UiObj, $RawVal, $PercText, $BottomText, $QueueRef, $LastRef, $TargetEnabled) 
    
    # 1. Filtra (Se Adaptive Ligado E TARGET HABILITADO)
    $FinalVal = $RawVal
    if ($Global:ConfigData.AdaptiveEnabled -and $TargetEnabled) {
        $FinalVal = Apply-AdaptiveFilter $RawVal $QueueRef $LastRef 
    }
...
$frontendInterval = if ($Global:ConfigData.AdaptiveEnabled) {
    290 # (300ms - 10ms overhead)
} elseif ($Global:ConfigData.FreqEnabled) { 
    $Global:ConfigData.FreqValue - 10 
} else { 
    290 # Padrão de fábrica: 300ms - 10ms overhead
}
if ($frontendInterval -lt 1) { $frontendInterval = 1 }

$timer = New-Object System.Windows.Forms.Timer; $timer.Interval = $frontendInterval

# --- LÓGICA DE CORTE DINÂMICO DE TELA (REFORÇADA) ---
$script:LayoutRedimensionado = $false
$script:AjustarLayoutCompacto = {
    if ($SyncHash.GpuActive -eq $false -and -not $script:LayoutRedimensionado) {
        $script:LayoutRedimensionado = $true
        
        # 1. Usa o objeto Size para forçar o Windows a encurtar a janela na hora
        $novaAltura = 110
        $formBack.Size = New-Object System.Drawing.Size($formBack.Width, $novaAltura)
        $form.Size = New-Object System.Drawing.Size($form.Width, $novaAltura)
        
        # 2. Esconde os blocos iterando com segurança (evita erros se algo não existir)
        $blocosParaEsconder = @($uiGpu, $uiVram)
        foreach ($bloco in $blocosParaEsconder) {
            if ($null -ne $bloco.Title) { $bloco.Title.Visible = $false }
            if ($null -ne $bloco.Frame) { $bloco.Frame.Visible = $false }
            if ($null -ne $bloco.LblVal) { $bloco.LblVal.Visible = $false }
            if ($null -ne $bloco.Symbol) { $bloco.Symbol.Visible = $false }
            if ($null -ne $bloco.LblBottom) { $bloco.LblBottom.Visible = $false }
        }
        
        # 3. Força a interface gráfica a redesenhar a janela instantaneamente
        $formBack.Refresh()
        $form.Refresh()
    }
}
...
$timer.Add_Tick({
    # 1. Verifica se precisa cortar a janela (GPU NVIDIA não detectada)
    & $script:AjustarLayoutCompacto

    # --- PARTE VISUAL (SÓ RODA SE A JANELA ESTIVER VISÍVEL) ---
    if (-not $Script:IsHidden) {
        & $script:UpdateBlock $uiCpu $SyncHash.CpuVal $SyncHash.CpuPercText $null $script:QueueCpu ([ref]$script:LastCpu) $Global:ConfigData.AdaptiveTargetCpu
        & $script:UpdateBlock $uiRam $SyncHash.RamVal $SyncHash.RamPercText $SyncHash.RamDetText $script:QueueRam ([ref]$script:LastRam) $Global:ConfigData.AdaptiveTargetRam
        & $script:UpdateBlock $uiGpu $SyncHash.GpuVal $SyncHash.GpuPercText $SyncHash.GpuTempText $script:QueueGpu ([ref]$script:LastGpu) $Global:ConfigData.AdaptiveTargetGpu
        & $script:UpdateBlock $uiVram $SyncHash.VramVal $SyncHash.VramPercText $SyncHash.VramDetText $script:QueueVram ([ref]$script:LastVram) $Global:ConfigData.AdaptiveTargetVram
        & $script:SyncOverlayPosition
    }
