...
* **Adaptive Sampling (Statistical Pipeline):** Includes advanced filters for data smoothing, such as *Rolling Average*, *EMA*, *DEMA*, *ALMA*, and *Hysteresis*.
...
2. Download the latest ZIP file (e.g., `PerformanceMonitor_v0.9_EXE.zip`).
...
1. Download the `PerformanceMonitor_v0.9.ps1` file.
...
3. To compile with the same scaling protection and behavior as the official release, use the following command:
   `ps2exe .\PerformanceMonitor_v0.9.ps1 -NoConsole -STA -SupportOS -DPIAware`
